...
def clean_html(html_content):
    allowed_tags = ['p', 'b', 'i', 'u', 'em', 'strong', 'a', 'br', 'ul', 'ol', 'li', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'blockquote', 'span', 'div', 'hr', 's']
    allowed_attrs = {'*': ['class'], 'a': ['href', 'target', 'rel']}
...
def index(request):
    thread_list = Thread.objects.filter(status=Thread.STATUS_PUBLISHED).order_by("-is_pinned", "-created_at")
...
def create_thread(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            thread.content = clean_html(thread.content)
            thread.save()
...
def edit_thread(request, thread_id):
    ...
    if request.user != thread.author:
        return HttpResponseForbidden("Chỉ tác giả mới có quyền chỉnh sửa bài viết này.")
...
def toggle_pin_thread(request, thread_id):
    ...
    if not request.user.is_admin():
        return HttpResponseForbidden("Chỉ Admin mới có quyền ghim bài viết.")
    
    thread.is_pinned = not thread.is_pinned
    thread.save()
    status = "Đã ghim" if thread.is_pinned else "Đã bỏ ghim"
    messages.success(request, f"{status} bài viết thành công.")
    return redirect("forum:detail", thread_id=thread.id)
